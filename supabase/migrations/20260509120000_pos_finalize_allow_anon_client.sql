-- POS calls `pos_finalize_order_stock` with the publishable key → JWT role is `anon`.
-- Previous gate allowed only `service_role` or `user_venues` for auth.uid(); anon always failed with forbidden.
-- Allow `anon` here (matches EXECUTE grant from 20260509010000); order/payment/line validation remains inside the RPC.

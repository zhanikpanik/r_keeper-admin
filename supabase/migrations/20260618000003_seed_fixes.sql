-- Fixes for demo seed: RLS on suppliers, RPC type errors, users cleanup
-- 1. Add RLS policy for suppliers inserts
DROP POLICY IF EXISTS "Allow all inserts" ON suppliers;
CREATE POLICY "Allow all inserts" ON suppliers FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "Allow all updates" ON suppliers;
CREATE POLICY "Allow all updates" ON suppliers FOR UPDATE USING (true);
DROP POLICY IF EXISTS "Allow all deletes" ON suppliers;
CREATE POLICY "Allow all deletes" ON suppliers FOR DELETE USING (true);

-- 2. Add users cleanup (non-org users only) to demo_clean_venue
...
BEGIN
  ...
  -- Clean demo users (keep base seed users with 00000000-* IDs)
  DELETE FROM users WHERE organization_id = v_org_id AND id NOT LIKE '00000000-%';
  
  RETURN 'ok';
END;
$$;

-- 3. Fix demo_gen_orders type errors
...
BEGIN
  ...
  LOOP
    ...
    v_shift_start := v_day + time '08:00' + (floor(random() * 15)::int::text || ' minutes')::interval;
    v_shift_end := v_day + time '16:00' + (floor(random() * 30)::int::text || ' minutes')::interval;
    v_shift_revenue := 0;
    v_shift_orders := 0;
    
    SELECT id INTO v_cashier_id FROM users WHERE organization_id = v_org_id AND role = 'cashier' ORDER BY random() LIMIT 1;
    
    FOR v_loop_idx IN 1..floor(v_num_orders * 0.55)::int LOOP
      ...
      v_opened_at := v_day + (lpad(v_hour::text, 2, '0') || ':' || lpad(v_min::text, 2, '0') || ':00')::time;
      v_closed_at := v_opened_at + (10 + floor(random() * 36)::int::text || ' minutes')::interval;
    ...
    FOR v_loop_idx IN 1..(1 + floor(random() * 3)::int) LOOP
      v_note := (ARRAY['payment_insert','Расход на продукты','Курьер','Хозтовары'])[1 + floor(random() * 4)::int];
      INSERT INTO cash_movements (venue_id, shift_id, movement_type, amount, note, occurred_at)
      VALUES (p_venue_id, v_shift_id, 'float_out', 200 + floor(random() * 1301)::numeric, v_note, v_shift_start + (1 + floor(random() * 6)::int::text || ' hours')::interval);
    END LOOP;
    
    -- ── EVENING SHIFT ──
    v_shift_id := gen_random_uuid();
    v_shift_start := v_day + time '16:00' + (floor(random() * 15)::int::text || ' minutes')::interval;
    v_shift_end := v_day + time '23:00' + (floor(random() * 30)::int::text || ' minutes')::interval;
    v_shift_revenue := 0;
    v_shift_orders := 0;
    
    SELECT id INTO v_cashier_id FROM users WHERE organization_id = v_org_id AND role = 'cashier' ORDER BY random() LIMIT 1;
    
    FOR v_loop_idx IN 1..ceil(v_num_orders * 0.45)::int LOOP
      v_order_num := v_order_num + 1;
      v_order_id := gen_random_uuid();

-- Heal demo data for ~1M som/month: fix statuses, redistribute dates, add payments & events.
-- Run: SELECT demo_heal_data('00000000-0000-0000-0000-000000000010');
...
BEGIN
  ...
  -- Keep ~5% of cancelled as cancelled, flip the rest to 'paid'
  WITH to_flip AS (
  ...
  -- ═══ 3. GENERATE MISSING PAYMENTS for paid orders ═══
  -- Delete existing payments first (will regenerate)
  ...
  -- ═══ 4. GENERATE MORE WAREHOUSE EVENTS (deliveries every 2 days) ═══
  v_new_deliveries := 0;
  FOR v_day IN SELECT generate_series(v_start_date, v_end_date, '2 day'::interval)::date LOOP
    ...
    v_time := v_day + time '09:00' + (floor(random() * 5)::text || ' hours')::interval
              + (floor(random() * 59)::text || ' minutes')::interval;
  ...
  -- ═══ 5. GENERATE WRITE-OFFS (every 3 days) ═══
  v_new_writeoffs := 0;
  FOR v_day IN SELECT generate_series(v_start_date, v_end_date, '3 day'::interval)::date LOOP
    ...
    v_time := v_day + time '21:00' + (floor(random() * 3)::text || ' hours')::interval
              + (floor(random() * 59)::text || ' minutes')::interval;
